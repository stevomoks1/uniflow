...
public class RequirementServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if ("batchFinalizeRequirements".equals(action)) {
                handleBatchFinalizeRequirements(request, response, user);
                return;
            }
    ...
    private void handleReleaseDraft(HttpServletRequest request, HttpServletResponse response, User user) throws IOException {
        ...
        if (requirementId <= 0 || isBlank(venue) || isBlank(dayOfWeek) || isBlank(timeSlot)) {
            response.sendRedirect(request.getContextPath() + "/timetableadmin/allocate_venue.jsp?error=invalid");
            return;
        }
        ...
        response.sendRedirect(request.getContextPath() + "/timetableadmin/allocate_venue.jsp?success=1");
    }
    ...
    private void handleFinalizeRequirement(HttpServletRequest request, HttpServletResponse response, User user) throws IOException {
        ...
        if (requirementId <= 0) {
            response.sendRedirect(request.getContextPath() + "/timetableadmin/manage_schedule.jsp?error=invalid");
            return;
        }

        WorkflowDAO.finalizeRequirement(requirementId, finalNote);
        response.sendRedirect(request.getContextPath() + "/timetableadmin/manage_schedule.jsp?success=1");
    }

    private void handleBatchFinalizeRequirements(HttpServletRequest request, HttpServletResponse response, User user) throws IOException {
        if (!"Timetabling Admin".equals(user.getRole())) {
            response.sendRedirect(request.getContextPath() + "/dashboard");
            return;
        }
        ...
        String finalNote = request.getParameter("finalNote");
        if (finalNote == null) {
            finalNote = "Batch finalized on " + new java.util.Date();
        }

        int finalizedCount = WorkflowDAO.batchFinalizeRequirements(finalNote);
        response.sendRedirect(request.getContextPath() + "/requirement/final_rquirements.jsp?batchFinalized=" + finalizedCount);
    }
    ...
    private void handleUpdateIssueStatus(HttpServletRequest request, HttpServletResponse response, User user) throws IOException {
        ...
        if (issueId <= 0 || isBlank(status)) {
            response.sendRedirect(request.getContextPath() + "/timetableadmin/view_issues.jsp?error=invalid");
            return;
        }
        ...
        response.sendRedirect(request.getContextPath() + "/timetableadmin/view_issues.jsp?success=1");
    }
