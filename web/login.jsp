...
<%
    ...
    String decodedLastUser = (String) request.getAttribute("decodedLastUser");
%>
<!DOCTYPE html>
<html lang="en">
<head>
...
<body>
    <main class="card">
        ...
        <% } %>
        <% if ((registered == null || registered.isEmpty()) && logout != null && !logout.isEmpty()) { %>
        <div class="flash" id="logoutMsg">You have been logged out successfully.</div>
        <% } %>
        <% if (error != null && !error.isEmpty()) { %>
        <div class="flash error"><%= error %></div>
        <% } %>
        <% if (decodedLastUser != null && !decodedLastUser.isEmpty()) { %>
        <div class="flash">Last signed in as <%= decodedLastUser %>.</div>
        <% } %>
        <script>
        (function() {
